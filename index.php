...
add_action( 'rcl_payments_gateway_init', 'rcl_add_alfabank_gateway' );
function rcl_add_alfabank_gateway() {
	rcl_gateway_register( 'alfabank', 'Rcl_Alfabank_Payment' );
}

class Rcl_Alfabank_Payment extends Rcl_Gateway_Core {
	...
	function __construct() {
		parent::__construct( array(
			'request'	 => 'alf-payment',
			'name'		 => 'Альфабанк',
			'submit'	 => __( 'Оплатить через Альфабанк' ),
	...
	function get_form( $data ) {
		...
		$login	 = rcl_get_commerce_option( 'alf_login' );
		$pass	 = rcl_get_commerce_option( 'alf_password' );

		if ( rcl_get_commerce_option( 'alf_test' ) ) {
			$action_adr = 'https://web.rbsuat.com/ab/rest/register.do';
		} else {
			$action_adr = 'https://pay.alfabank.ru/payment/rest/register.do';
		}
		...
		$args = array(
			...
			'failUrl'		 => add_query_arg( array( 'alf-login' => $login ), get_permalink( $data->page_fail ) ),
		...
		if ( rcl_get_commerce_option( 'alf_fn' ) ) {

			$args['taxSystem'] = rcl_get_commerce_option( 'alf_tax' );
			...
			if ( $data->pay_type == 1 ) {
				...
				$items[] = array(
					...
					'itemPrice'		 => 1 * 100,
					'tax'			 => [
						'taxType' => rcl_get_commerce_option( 'alf_nds' ),
					],
	...
	function result( $data ) {
		...
		if ( isset( $_REQUEST['alf-payment'] ) AND $_REQUEST['alf-payment'] == $login ) {

			if ( rcl_get_commerce_option( 'alf_test' ) ) {
				$action_adr = 'https://web.rbsuat.com/ab/rest/getOrderStatusExtended.do';
			} else {
				$action_adr = 'https://pay.alfabank.ru/payment/rest/getOrderStatusExtended.do';
			}
			...
			if ( $response['errorCode'] ) {
				rcl_add_log( 'alf-result', [$args, $response ], 1 );
			} else if ( $response['orderStatus'] == '1' || $response['orderStatus'] == '2' ) {
